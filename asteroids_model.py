...
class Asteroid:
    def __init__(self,_x,_y,_speed): #defines asteroid
        self.x = _x
        self.y = _y
        self.speed = _speed
...
class Game:
    def __init__(self, _dis_x, _dis_y): # pass game settings as parameters
        ...
        self.SPAWN_RARETY = 60  * self.DIS_X * self.DIS_Y
    ...
    def spawn_asteroids(self):
        ...
        if get_time_in_ms() - self.last_spawn >= self.POSSIBLE_SPAWN_RATE and randint(0,100) < self.SPAWN_RARETY:
            asteroid = Asteroid(randint(0,self.DIS_X-1), 0, randint(self.ASTEROID_MIN_SPEED, self.ASTEROID_MAX_SPEED))
